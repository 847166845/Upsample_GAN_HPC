...
import numpy as np
from .dataset import CUDAPrefetcher, TrainValidHRTFDataset, CPUPrefetcher, MergeHRTFDataset
...
def spectral_distortion_inner(input_spectrum, target_spectrum, eps=1e-12):
    numerator = target_spectrum
    denominator = input_spectrum
    log_value = 20 * torch.log10(torch.clamp(numerator / denominator, min=eps))
    return torch.mean(log_value ** 2)
    # numerator = target_spectrum
    # denominator = input_spectrum
...
def ILD_metric_inner(config, input_spectrum, target_spectrum, eps=1e-12):
    ...
    input_ILD = torch.mean(20 * torch.log10(torch.clamp(input_left / input_right, min=eps)))
    target_ILD = torch.mean(20 * torch.log10(torch.clamp(target_left / target_right, min=eps)))
...
def sd_ild_loss(config, generated, target, sd_mean, sd_std, ild_mean, ild_std):
    ...
    # Check for NaNs after calculating the metrics
    if torch.isnan(sd_metric).any():
        print("NaNs found in sd_metric")
    if torch.isnan(ild_metric).any():
        print("NaNs found in ild_metric")
    ...
    # Check for NaNs after the division
    if torch.isnan(sd_norm).any():
        print("NaNs found in sd_norm")
    if torch.isnan(ild_norm).any():
        print("NaNs found in ild_norm")

    # add normalized metrics together
    sum_norms = torch.add(sd_norm, ild_norm)

    # Check for NaNs after the addition
    if torch.isnan(sum_norms).any():
        print("NaNs found in sum_norms")

    # un-normalize
    sum_std = (sd_std ** 2 + ild_std ** 2) ** 0.5
    sum_mean = sd_mean + ild_mean
    # if torch.isnan(sum_std).any():
    #     print("NaNs found in sum_std")
